...
@param {AthomCloudAPI.StorageAdapter} [opts.store={@link AthomCloudAPI.StorageAdapterBrowser} or {@link AthomCloudAPI.StorageAdapterMemory}]`;
  ...
  constructor({
    ...
    this.__store = store;
    this.__storeOperationPromise = Promise.resolve();
    ...
    this.__token = token;
    this.__user = null;
    this.__delegationTokenKeyCache = null;
    this.__delegationTokenKeyPromise = null;
  }
  ...
  */
  ...
  /**
   * Verify a delegation token locally. The public key is cached in memory for one hour.
   * @param {object} opts
   * @param {string} opts.token
   * @param {string} [opts.audience]
   * @returns {Promise<object>}
   */
  async verifyDelegationTokenCached({ token, audience } = {}) {
    if (Util.isBrowser() || Util.isReactNative()) {
      throw new Error('verifyDelegationTokenCached() is not supported in browsers.');
    }

    let keyCache = this.__delegationTokenKeyCache;

    if (keyCache == null || keyCache.expiresAt <= Date.now()) {
      if (this.__delegationTokenKeyPromise == null) {
        this.__delegationTokenKeyPromise = this.getDelegationTokenKey()
          .then((publicKey) => {
            const result = {
              publicKey,
              expiresAt: Date.now() + (60 * 60 * 1000),
            };
            this.__delegationTokenKeyCache = result;
      ...
      keyCache = await this.__delegationTokenKeyPromise;
    }

    try {
      return jwt.verify(token, keyCache.publicKey, {
        algorithms: ['RS512'],
        audience: audience || undefined,
      });
    } catch (err) {
      throw new APIError('The provided token is invalid.', 401);
    }
  }
  ...
  */
  ...
  async __getStore() {
    ...
    await this.__storeOperationPromise;

    return await this.__store.get();
  }

  async __setStore(value) {
    await this.__updateStore((store) => {
      return {
        ...store,
        ...value,
      };
  ...
  async __updateStore(update) {
    if (!this.__store) return;

    await this.__enqueueStoreOperation(async () => {
      const store = await this.__store.get();
      const updatedStore = update(store);

      await this.__store.set(updatedStore);
    });
  }
  ...
  async __enqueueStoreOperation(operation) {
    const operationPromise = this.__storeOperationPromise.then(async () => {
      await operation();
    });

    // Keep later operations usable when this operation fails. Callers still
    // receive the rejection through operationPromise below.
    this.__storeOperationPromise = operationPromise.catch(() => {});
