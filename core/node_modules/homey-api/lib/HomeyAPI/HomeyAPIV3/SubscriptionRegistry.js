...
class SubscriptionRegistry {
  ...
  __onSessionReconnect() {
    for (const entry of this.__entries.values()) {
      ...
      this.__resumeEntry(entry).catch(() => {});
    }
  }
  ...
  __onSessionReadyError(err) {
    for (const entry of this.__entries.values()) {
      ...
      this.__notifyConsumersSafely(entry, 'onReconnectError', err);
    }
  }
  ...
  __resumeEntry(entry, revision = null) {
    ...
    const reconnectPromise = (async () => {
      ...
      this.__notifyConsumersSafely(entry, 'onReconnect');
    })();
  ...
  __notifyConsumersSafely(entry, event, ...args) {
    for (const consumer of entry.consumers) {
      if (!consumer.active) continue;

      try {
        const handlerResult = consumer.handlers[event](...args);

        Promise.resolve(handlerResult)
          .catch((err) => {
            this.homey.__debug(`Subscription ${event} Handler Error`, entry.uri, err);
          })
          .catch(() => {});
      } catch (err) {
        this.homey.__debug(`Subscription ${event} Handler Error`, entry.uri, err);
